...
import { deletePost, changePostStatus } from "../../../../_actions/admin/post.action";
...
const Post = () => {
  ...
  useEffect(() => {
    ...
    $('body').on('click', '.edit', function (e) {
      e.preventDefault();
      history.push('/admin/post/' + $(this).data('id') + '/edit')
    });
    ...
    $('body').on('click', '.delete', function (e) {
      ...
      $(".delete-modal-btn")
        .off()
        .on("click", function () {
          dispatch(deletePost(sid)).then(res => {
    ...
    $('#datatable').DataTable({
      ...
      "ajax": {
        ...
        dataFilter: function (response) {
          ...
          json.data = data.responseData.data.posts;
          console.log(json);
    ...
    $('body').on('change', '.status_enable', function () {
      ...
      dispatch(changePostStatus(id, value)).then(res => {
        addToast(res.message, { appearance: res.status, autoDismiss: true, })
